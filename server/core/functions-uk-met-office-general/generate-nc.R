...
params <- c(
  ...
  "precipitation-accumulation-6h"
)
timesteps.nc <- seq(
  as.POSIXct(strptime(str.date, "%Y%m%d")),
  by = "1 hour",
  length = 6 * 24 + 1
)
...
list.df.forecast <- lapply(paths.json, function(x) {
  ...
  if(x.param %in% c("relative-humidity", "temperature")) {
    x.spline.xout <- seq.POSIXt(
      x.timesteps_json.posixct[1],
      x.timesteps_json.posixct[length(x.timesteps_json.posixct)],
      by = "min"
    ); xi.timesteps.nc <- sapply(
      format(timesteps.nc, "%Y%m%d%H%M"),
      function(x) which(x == format(x.spline.xout, "%Y%m%d%H%M")),
      USE.NAMES = FALSE
    )

    x.data <- data.frame(
      t = x.spline.xout[xi.timesteps.nc],
      as.data.frame(apply(x.df.coords, 1, function(row) {
        spline(
          x.timesteps_json.posixct,
          sapply(x.json$data, function(t) t[[ row[["y"]] ]][[ row[["x"]] ]]),
          xout = x.spline.xout,
          method = "natural"
        )$y[xi.timesteps.nc]
      }) - if(x.param == "temperature") 273.15 else 0)
    )
  } else if(x.param == "precipitation-accumulation-1h") {
  ...
  } else if(x.param == "precipitation-accumulation-6h") {
    x.timesteps_json.posixct.hourly <- c(); for(t in seq_along(x.timesteps_json.posixct)) x.timesteps_json.posixct.hourly <- c(
      x.timesteps_json.posixct.hourly,
      x.timesteps_json.posixct[t] - 5 * 3600,
      x.timesteps_json.posixct[t] - 4 * 3600,
      x.timesteps_json.posixct[t] - 3 * 3600,
      x.timesteps_json.posixct[t] - 2 * 3600,
      x.timesteps_json.posixct[t] - 1 * 3600,
      x.timesteps_json.posixct[t] - 0 * 3600
    ); rm(t)
    ...
    x.data <- data.frame(
      t = as.POSIXct(x.timesteps_json.posixct.hourly, origin = '1970-01-01'),
      as.data.frame(sapply(
        apply(x.df.coords, 1, function(row) x.json$data[[ row[["y"]] ]][[ row[["x"]] ]]),
        function(col) rep(col/3, each = 6)
      ))
    )
  }
...
df.climate_vars <- data.frame(
  ...
  d = c(
    ...
    "Temperature (at 1.5 m above ground)",
    "Total Rainfall"
  ),
  u = c("%", "deg_C", "mm"),
  p = c("float", "float", "float")
)
  ...
  v.df <- if(df.climate_vars$v[v] == "prec") rbind(
    cbind(
      ...
      as.data.frame(t(sapply(
        colnames(list.df.forecast[[3]]$df)[-1], function(col) 0, USE.NAMES = TRUE
      )))
    ),
    list.df.forecast[[3]]$df,
    list.df.forecast[[4]]$df,
    list.df.forecast[[5]]$df
  ) else if(
    df.climate_vars$v[v] %in% c("rh2m", "t2m")
  ) list.df.forecast[[v]]$df else NULL
  v.df <- v.df[1:121,]
