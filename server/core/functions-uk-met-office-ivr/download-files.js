...
module.exports = files => new Promise((resolve, reject) => {
  ...
  log("Getting timestep metadata", "METOFFICEIVR_DOWNLOADFILES");
  Promise.all(
    files.map(el => axios({
    ...
    log("Attempting to download new .grib2 files", "METOFFICEIVR_DOWNLOADFILES");
    const
    ...
    log("New .grib2 files downloaded", "METOFFICEIVR_DOWNLOADFILES");
    resolve(filesData);
  }).catch(err => {
    reject(err);
  })
